...
*/
vector<char*> FilterParamsValidatorFactory::GetFilterParamsRaw(char** argValues, ushort& currentIndex, const ushort& numberOfParams)
{
	...
	char** argValues, ushort& currentIndex, const cv::Mat& image)
{
	...
	case ImageFilterTypesEnum::INVALID_FILTER_TYPE:
		return nullptr;
	case RESIZE:
		return new ResizeFilterParamsValidator(GetFilterParamsRaw(argValues, currentIndex, 2));
	case ROTATE:
		return new RotateFilterParamsValidator(GetFilterParamsRaw(argValues, currentIndex, 2));
	case FLIP:
		return new FlipFilterParamsValidator(GetFilterParamsRaw(argValues, currentIndex, 1));
	case CROP:
		return new CropFilterParamsValidator(GetFilterParamsRaw(argValues, currentIndex, 4), image);
	case RGB_TO_GRAYSCALE:
		currentIndex += 1;
		return new FilterParamsValidator();
	case BRIGHTNESS_ADJ:
		return new BrightnessAdjFilterParamsValidator(GetFilterParamsRaw(argValues, currentIndex, 1));
	default:
		return nullptr;
	}
	return nullptr;
